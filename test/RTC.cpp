...
void RTC_DS3231::adjust(const DateTime& dt)
{
  ...
  twi.write((uint8_t)0); // start at location 0
  twi.write(bin2bcd(dt.second()));
  twi.write(bin2bcd(dt.minute()));
  twi.write(bin2bcd(dt.hour()));
  twi.write(bin2bcd(0));
  twi.write(bin2bcd(dt.day()));
  twi.write(bin2bcd(dt.month()));
  twi.write(bin2bcd(dt.year() - 2000));
  twi.endTransmission();
...
DateTime RTC_DS3231::now()
{
  ...
  twi.requestFrom(DS3231_ADDRESS, 7);
  uint8_t ss = bcd2bin(twi.read() & 0x7F);
  uint8_t mm = bcd2bin(twi.read());
  uint8_t hh = bcd2bin(twi.read());
  twi.read();
  uint8_t d = bcd2bin(twi.read());
  uint8_t m = bcd2bin(twi.read());
  uint16_t y = bcd2bin(twi.read()) + 2000;
...
Ds3231SqwPinMode RTC_DS3231::readSqwPinMode()
{
  ...
  twi.beginTransmission(DS3231_ADDRESS);
  twi.write(DS3231_CONTROL);
  twi.endTransmission();

  twi.requestFrom((uint8_t)DS3231_ADDRESS, (uint8_t)1);
  mode = twi.read();
...
float RTC_DS3231::getTemp()
{
  ...
  twi.beginTransmission(DS3231_ADDRESS);
  twi.write(DS3231_TEMP);
  twi.endTransmission();

  twi.requestFrom((uint8_t)DS3231_ADDRESS, (uint8_t)2);
  temp_msb = twi.read();
  temp_lsb = twi.read() >> 6;
  twi.endTransmission();
...
void RTC_DS3231::alarmInterrupt(uint8_t alarmNumber, bool interruptEnabled)
{
    ...
    twi.beginTransmission(DS3231_ADDRESS);
    twi.write(DS3231_CONTROL);
    controlReg = twi.endTransmission();
    if (!controlReg) {
      twi.requestFrom((uint8_t)DS3231_ADDRESS, (uint8_t)1);
      controlReg = twi.read();
      twi.endTransmission();
