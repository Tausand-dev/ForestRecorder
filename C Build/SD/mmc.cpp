...
#define CS_LOW()		digitalWrite(&CS_SD_PORT, CS_SD, 0)	/* Set MMC_CS = low */
#define	CS_HIGH()		digitalWrite(&CS_SD_PORT, CS_SD, 1)	/* Set MMC_CS = high */
...
#define	FCLK_SLOW()		SPCR = (1 << SPE) | (1 << MSTR) | (1 << SPR1); //Enable SPI, Set as Master, Prescaler: Fosc/16	/* Set SPI clock for initialization (100-400kHz) */
#define	FCLK_FAST()		SPCR = (1 << SPE) | (1 << MSTR); // 4 MHz /* Set SPI clock for read/write (20MHz max) */
...
void power_on (void)
{
	...
	SPCR |= (1 << SPE) | (1 << MSTR); //Enable SPI, Set as Master, Prescaler: Fosc/16
...
static BYTE xchg_spi (BYTE dat) 	/* Data to be sent */
{
	SPDR = dat;
