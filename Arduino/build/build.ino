...
//    RTC.adjust(DateTime(2018, 6, 14, 14, 30, 0));
    ...
    attachInterrupt(INT0, wakeUpAlarm, FALLING);
...
void uartFunc(void)
{
    ...
    while (true)
    {
        ...
        if (waiting > 0)
        {
            ...
            if ((incomingByte == 0)  & (waiting > 12))
            {
                timeTemp[0] = Serial.read();
                timeTemp[1] = Serial.read();
                day = atoi(timeTemp);
                ...
                timeTemp[0] = Serial.read();
                timeTemp[1] = Serial.read();
                minute = atoi(timeTemp);
    
                timeTemp[0] = Serial.read();
                timeTemp[1] = Serial.read();
                second = atoi(timeTemp);
